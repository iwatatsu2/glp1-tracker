...
export function WeightDosageGraph() {
  ...
  const currentWeight = mockWeightRecords[mockWeightRecords.length - 1]?.weight || 0
  const startWeight = mockWeightRecords[0]?.weight || 0
  ...
  const chartData = mockWeightRecords.map(r => ({
    date: formatDate(r.date),
    weight: r.weight,
    dosage: r.dosage
  }))

  const dosageChanges = mockWeightRecords
  ...
  const handleSaveWeight = () => {
    if (newWeight) {
      console.log('Saving weight:', parseFloat(newWeight))
      setNewWeight("")
    }
  }

  const dosageColors: Record<number, string> = {
    0.25: 'rgba(59, 130, 246, 0.15)',
    0.5: 'rgba(139, 92, 246, 0.15)',
    1.0: 'rgba(34, 197, 94, 0.15)',
    1.7: 'rgba(245, 158, 11, 0.15)',
    2.4: 'rgba(239, 68, 68, 0.15)',
    2.5: 'rgba(139, 92, 246, 0.12)',
    5: 'rgba(139, 92, 246, 0.18)',
    7.5: 'rgba(34, 197, 94, 0.15)',
    10: 'rgba(245, 158, 11, 0.15)',
    12.5: 'rgba(239, 68, 68, 0.12)',
    15: 'rgba(239, 68, 68, 0.18)',
  }

  const uniqueDosages = [...new Set(mockWeightRecords.map(r => r.dosage))].sort()
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        {MEDICATIONS.map(med => (
          <button
            ...
            className={cn(
              "flex-1 py-2.5 px-3 rounded-xl text-sm font-medium transition-all border-2",
              medication === med.id
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card className={cn("bg-gradient-to-b", isWeightLoss ? "from-green-50 to-white" : "from-red-50 to-white")}>
          <CardContent className="pt-4 pb-3 text-center">
            {isWeightLoss ? (
              <TrendingDown className="size-5 text-success mx-auto mb-1" />
            ) : (
              <TrendingUp className="size-5 text-destructive mx-auto mb-1" />
            )}
            <p className={cn(
              "text-2xl font-bold",
              isWeightLoss ? "text-success" : "text-destructive"
            )}>
      ...
      <Card className="bg-gradient-to-b from-slate-50/50 to-white">
        <CardHeader className="pb-2">
          <CardTitle className="text-base">体重推移と用量</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="h-64">
              ...
              <LineChart data={chartData} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
                {chartData.map((entry, index) => {
                  ...
                  if (prevEntry.dosage !== entry.dosage || index === chartData.length - 1) {
                    ...
                    while (startIndex > 0 && chartData[startIndex - 1].dosage === prevEntry.dosage) {
                      startIndex--
                    }
                    return (
                      <ReferenceArea
                        ...
                        x2={entry.date}
                        fill={dosageColors[prevEntry.dosage] || 'rgba(0,0,0,0.05)'}
                      />
                    )
                  }
                  return null
                ...
                {dosageChanges.slice(1).map((change, index) => (
                  <ReferenceLine
                    key={index}
                    x={change.date}
                    stroke={medInfo.color}
                    strokeDasharray="4 4"
                    label={{
                      value: `${change.dosage}mg`,
                      position: 'top',
                      fontSize: 10,
                      fill: medInfo.color,
                      offset: 5,
                ...
                <Line
                  ...
                  stroke={medInfo.color}
                  strokeWidth={2.5}
                  dot={{ fill: medInfo.color, strokeWidth: 0, r: 4 }}
                  activeDot={{ r: 6, fill: medInfo.color }}
            ...
            {uniqueDosages.map(dosage => (
              <div key={dosage} className="flex items-center gap-1">
                <div
                  ...
                  style={{ backgroundColor: dosageColors[dosage]?.replace('0.1', '0.4').replace('0.15', '0.4').replace('0.12', '0.4').replace('0.18', '0.5') }}
                />
      ...
      <Card className="bg-slate-50/50">
        ...
        <CardContent>
          <div className="space-y-3">
            {[...mockWeightRecords].reverse().map((record, index, arr) => {
